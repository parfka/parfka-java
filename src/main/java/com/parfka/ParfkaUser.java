...
import com.parfka.callback.LoginCallback;
import com.parfka.callback.RequestPasswordResetCallback;
import com.parfka.callback.SignUpCallback;
import com.parfka.command.*;
...
public class ParfkaUser extends ParfkaObject {
	
	public static ParfkaUser currentUser;
	...
	public ParfkaUser() {
		super(ParseRegistry.getClassName(ParfkaUser.class));
		setEndPoint("users");
	}
	...
	public static ParfkaUser logIn(String username, String password) throws ParseException {
		ParfkaUser pu = new ParfkaUser();
	...
	void validateSave() {
		...
		if (getObjectId() == null) {
			LOGGER.error("Cannot save a ParfkaUser until it has been signed up. Call signUp first.");
			throw new IllegalArgumentException(
					"Cannot save a ParfkaUser until it has been signed up. Call signUp first.");
		}

		if ((!isAuthenticated()) && isDirty && getObjectId() != null) {
			LOGGER.error("Cannot save a ParfkaUser that is not authenticated.");
			throw new IllegalArgumentException(
					"Cannot save a ParfkaUser that is not authenticated.");
		}
	...
	public void signUp() throws ParseException {
		...
		if(!response.isFailed()) {
			...
			try {
				...
				LOGGER.error("Although Parfka reports object successfully saved, the response was invalid.");
	...
	public static ParfkaUser login(String username, String password) throws ParseException {
		...
		if(!response.isFailed()) {
			...
			try {
				ParfkaUser parseUser = new ParfkaUser();
				...
				parseUser.setCreatedAt(Parfka.parseDate(createdAt));
				parseUser.setUpdatedAt(Parfka.parseDate(updatedAt));
