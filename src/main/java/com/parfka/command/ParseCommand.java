...
import static com.parfka.ParseConstants.*;
...
public abstract class ParseCommand {
	...
	protected void setupHeaders(HttpRequestBase requestBase, boolean addJson) {
		requestBase.addHeader(HEADER_APPLICATION_ID, Parfka.getApplicationId());

		if (Parfka.isIsRootMode()) {
			requestBase.addHeader(HEADER_MASTER_KEY, Parfka.getMasterKey());
		} else {
			requestBase.addHeader(HEADER_REST_API_KEY, Parfka.getRestAPIKey());
		}
		...
		} else if (ParfkaUser.currentUser != null) {
			// if we are logged in, pass the session token
			requestBase.addHeader(HEADER_SESSION_TOKEN,
					ParfkaUser.currentUser.getSessionToken());
		}
	}
