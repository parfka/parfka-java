...
import com.parfka.callback.CountCallback;
import com.parfka.callback.FindCallback;
...
public class ParseQuery<T extends ParfkaObject> {
	...
	public static <T extends ParfkaObject> ParseQuery<T> getQuery(
			Class<T> subclass) {
	...
	public ParseQuery<T> whereRelatedTo(ParfkaObject parent, String key) {
	...
	public JSONObject toREST() {
		...
		try {
			...
			if (!this.include.isEmpty()) {
				params.put("include", Parfka.join(this.include, ","));
			}
			
			if (this.selectedKeys != null) {
				params.put("keys", Parfka.join(this.selectedKeys, ","));
			}
	...
	public class RelationConstraint {
		private String key;
		private ParfkaObject object;

		public RelationConstraint(String key, ParfkaObject object) {
		...
		public ParfkaObject getObject() {
			return this.object;
		}

		public ParseRelation<ParfkaObject> getRelation() {
