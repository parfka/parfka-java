...
import com.parfka.ParseRelation;
import com.parfka.encode.ParseObjectEncodingStrategy;
import com.parfka.util.ParseEncoder;

public class RelationOperation<T extends ParfkaObject> implements ParseFieldOperation {

	private String targetClass;
	private Set<ParfkaObject> relationsToAdd;
	private Set<ParfkaObject> relationsToRemove;
	
	public RelationOperation(Set<T> newRelationsToAdd, Set<T> newRelationsToRemove) {
		this.targetClass = null;
		this.relationsToAdd = new HashSet<ParfkaObject>();
		this.relationsToRemove = new HashSet<ParfkaObject>();

		if (newRelationsToAdd != null) {
			for (ParfkaObject object : newRelationsToAdd) {
		...
		if (newRelationsToRemove != null) {
			for (ParfkaObject object : newRelationsToRemove) {
	...
	private void addParseObjectToSet(ParfkaObject obj, Set<ParfkaObject> set) {
		...
		for (ParfkaObject existingObject : set) {
	...
	public JSONArray convertSetToArray(Set<ParfkaObject> set, ParseObjectEncodingStrategy objectEncoder) throws JSONException {
		JSONArray array = new JSONArray();
		for (ParfkaObject obj : set) {
	...
	public Object apply(Object oldValue, ParfkaObject parfkaObject, String key) {
		...
		if (oldValue == null) {
			relation = new ParseRelation<T>(parfkaObject, key);
